...
%%Anexo B. CÓDIGO MEDELLÍN

%% EXPLORANDO EL TIPO DE DATOS
 
% Data type
info = georasterinfo('Topo_medellín.asc');
 
% Data reading and mapping 
[A,R,cmap] = readgeoraster('Topo_medellín.asc','CoordinateSystemType','geographic', 'OutputType','double');
...
dz_dx = zeros([552 584]);
dz_dy=zeros([552 584]);
riserun=zeros([552 584]);
slopedegrees=zeros([552 584]);
...
filtred3 = zeros([550 582]);
for i = 1:s1
    for j = 1:s2
       if  filtred2(i,j) < 50 %insert the same epsilon%
           filtred3(i,j)= 1;
             else
...
filtred4 = zeros([550 582]);
for i = 1:s1
    for j = 1:s2
       if  filtred3(i,j)> 50 %insert the same epsilon%
           filtred4(i,j)= 2;
             else
...
geoshow(filtred5,R,'DisplayType','surface')%save the plot for the epsilon
title('Slope Data')
colorbar
surf(filtred5)
 
%Binary Se binariza la matriz en valores entre 0  y 1 
binary = zeros([550 582]);
for i = 1:s1
    for j = 1:s2
        if filtred5(i,j) ~= 2
            binary(i,j)= 0;
...
S = sum(TF); %contar isNaN
P = sum(S); %Numero total de NaN
n = N - P;
 
geoshow(F,R,'DisplayType','surface')%save the plot for the epsilon as slope
title('Slope Data')
colorbar
surf(F)%save the plot for the epsilon as DEM
 
%contador de cluster guardar PixelIdxlist
BW = zeros([550 582]);
BW = binary;
CC = bwconncomp(BW);
%Trasponer y concatenar PixelIdxList
